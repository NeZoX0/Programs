...
<head>
    ...
    <title>Register Manyfactures</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <h2>Таблиця Manyfactures</h2>
    ...
    if($result = $mysqli->query('SELECT * FROM manufacturer')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<table><tr><th>Код</th><th>Ім'я Компанії</th>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td>",$row['code'],$row['name_company']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
		<li><a href="plustablemanufacturesform.php">Вставити рядок</a><br></li>
		<li><a href="updatetablemanufacturesform.php">Оновити рядок</a><br></li>
		<li><a href="minustablemanufacturesform.php">Видалити рядок</a><br></li>
