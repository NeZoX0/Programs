...
<body>
    ...
    if($result = $mysqli->query('SELECT * FROM program')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<table><tr><th>ІД</th><th>Ім'я</th><th>Опис</th><th>Розмір програми</th>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td>",$row['id'],$row['name'],$row['opys'],$row['size']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
		<li><a href="plustableprogramform.php">Вставити рядок</a><br></li>
		<li><a href="updatetableprogramform.php">Оновити рядок</a><br></li>
		<li><a href="minustableprogramform.php">Видалити рядок</a><br></li>
